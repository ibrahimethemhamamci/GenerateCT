import os
import glob
import json
import torch
import pandas as pd
import numpy as np
from PIL import Image
import nibabel as nib
from torch.utils.data import Dataset
import torchvision.transforms as transforms
from functools import partial

def cast_num_frames(t, *, frames):
    f = t.shape[1]
    if f%frames==0:
        return t[:,:-(frames-1)]
    if f%frames==1:
        return t
    else:
        return t[:,:-((f%frames)-1)]


class VideoTextDataset(Dataset):
    def __init__(self, data_folder, xlsx_file, min_slices=20, resize_dim=256, num_frames=2, force_num_frames=True):
        self.data_folder = data_folder
        self.min_slices = min_slices
        self.accession_to_text = self.load_accession_text(xlsx_file)
        self.samples = self.prepare_samples()
        self.resize_dim = resize_dim
        self.resize_transform = transforms.Resize((resize_dim, resize_dim))
        self.transform = transforms.Compose([
            transforms.Resize((resize_dim,resize_dim)),
            #T.RandomHorizontalFlip() if horizontal_flip else T.Lambda(identity),
            #T.CenterCrop(image_size),
            transforms.ToTensor()
        ])
        self.nii_to_tensor = partial(self.nii_img_to_tensor, transform = self.transform)
        self.cast_num_frames_fn = partial(cast_num_frames, frames = num_frames) if force_num_frames else identity

    def load_accession_text(self, xlsx_file):
        df = pd.read_excel(xlsx_file)
        accession_to_text = {}
        for index, row in df.iterrows():
            accession_to_text[row['AccessionNo']] = row['Impressions']
        return accession_to_text

    def prepare_samples(self):
        samples = []

        protocol_translation = {
            'TORAKS KONTRASTSIZ/TORAKS': 'Chest CT unenhanced',
            # Add more translations as needed
        }

        for patient_folder in glob.glob(os.path.join(self.data_folder, '*')):
            for accession_folder in glob.glob(os.path.join(patient_folder, '*')):
                accession_number = os.path.basename(accession_folder)
                if accession_number not in self.accession_to_text:
                    continue

                impression_text = self.accession_to_text[accession_number]

                for nii_file in glob.glob(os.path.join(accession_folder, '*.nii.gz')):
                    nii_img = nib.load(nii_file)
                    if nii_img.shape[-1] < 257 or nii_img.shape[-1] > 600:
                        continue
                    else:
                        # Load metadata file
                        metadata_file = os.path.splitext(nii_file)[0][:-4]+ '_metadata.json'
                        with open(metadata_file, 'r') as f:
                            metadata = json.load(f)

                        # Extract required metadata
                        age = metadata['PatientAge'][:-1].zfill(3)
                        sex = metadata['PatientSex']
                        protocol = metadata['ProtocolName']

                        # Translate protocol to English
                        protocol_english = protocol_translation.get(protocol, protocol)

                        # Construct the input text with the included metadata
                        input_text = f'{age} years old {sex} {protocol_english}: impression section {impression_text}'

                        samples.append((nii_file, input_text))

        return samples

    def __len__(self):
        return len(self.samples)

    def nii_img_to_tensor(self, path, transform):
        nii_img = nib.load(str(path))
        img_data = nii_img.get_fdata()
        img_data = img_data - 1024
        hu_min, hu_max = -1000, 1000
        img_data = np.clip(img_data, hu_min, hu_max)

        img_data = ((img_data + 1000) * (1 / 2000)).astype(np.float32)

        slices = []

        for i in range(200):
            img_slice = Image.fromarray(img_data[:, :, i], mode='F')
            img_transformed = transform(img_slice)
            #slices.append(torch.tensor(np.float16(img_transformed), dtype=torch.float16))
            slices.append(img_transformed)
        return torch.stack(slices, dim=1)
    
    def __getitem__(self, index):
        nii_file, input_text = self.samples[index]
        video_tensor = self.nii_to_tensor(nii_file)

        
        #img_data = img_data.transpose(2, 0, 1)  # Rearrange dimensions to (channels, frames, height, width)

        # Resize each frame using the resize_transform
        resized_data = []
        
        
        input_text = input_text.replace('"', '')  
        input_text = input_text.replace('\'', '')  
        input_text = input_text.replace('(', '')  
        input_text = input_text.replace(')', '')  
        print(f"input_text: {input_text}")

        return video_tensor, input_text
