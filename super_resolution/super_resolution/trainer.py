...
from super_resolution.superres_pytorch import Superres, NullUnet
from super_resolution.elucidated_superres import ElucidatedSuperres
from super_resolution.data import cycle
...
# super_resolution trainer

def super_resolution_sample_in_chunks(fn):
    @wraps(fn)
    def inner(self, *args, max_batch_size = None, **kwargs):
        if not exists(max_batch_size):
            return fn(self, *args, **kwargs)

        if self.super_resolution.unconditional:
...
class SuperResolutionTrainer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert isinstance(superres, (Superres, ElucidatedSuperres))
        ema_kwargs, kwargs = groupby_prefix_and_trim('ema_', kwargs)

        # elucidated or not

        self.is_elucidated = isinstance(superres, ElucidatedSuperres)
        ...
        # superresolution, unets and ema unets

        self.superres = superres
        self.num_unets = len(self.superres.unets)
        ...
        for ind, (unet, unet_lr, unet_eps, unet_warmup_steps, unet_cosine_decay_max_steps) in enumerate(zip(self.superres.unets, lr, eps, warmup_steps, cosine_decay_max_steps)):
            optimizer = Adam(
                unet.parameters(),
                lr = unet_lr,
                eps = unet_eps,
    ...
    def validate_and_set_unet_being_trained(self, unet_number = None):
        ...
        self.superres.only_train_unet_number = unet_number
    ...
    def wrap_unet(self, unet_number):
        ...
        unet = self.superres.get_unet(unet_number)
        unet_index = unet_number - 1
    ...
    def print_untrained_unets(self):
        ...
        for ind, (steps, unet) in enumerate(zip(self.steps.tolist(), self.superres.unets)):
    ...
    def add_train_dataset(self, ds = None, *, batch_size, **dl_kwargs):
        ...
        self.add_valid_dataset(valid_ds, batch_size = batch_size, shuffle=True, **dl_kwargs)
    ...
    def step_with_dl_iter(self, dl_iter, **kwargs):
        dl_tuple_output = list(next(dl_iter))
        dl_tuple_output[0]=dl_tuple_output[0].permute(0,2,1,3,4)[0]
        dl_tuple_output[1]=dl_tuple_output[1].permute(0,2,1,3,4)[0]
        dl_tuple_output[2]=str(dl_tuple_output[2][0])
        dl_tuple_output[2] = [dl_tuple_output[2] for i in range(dl_tuple_output[0].shape[0])]
        #dl_tuple_output[1]=[dl_tuple_output[1]]
        #dl_tuple_output[0]=dl_tuple_output[0][10*i:10*(i+1)]
        #dl_tuple_output[1]=dl_tuple_output[1][10*i:10*(i+1)]
        #dl_tuple_output[2]=dl_tuple_output[2][10*i:10*(i+1)]
        dl_tuple_output_casted=cast_tuple(dl_tuple_output)
        model_input = dict(list(zip(self.dl_tuple_output_keywords_names, dl_tuple_output_casted)))
        #print(model_input["images"].shape)
        #print(model_input["texts"])
    ...
    def save(
        ...
        **kwargs
        ...
        save_obj = dict(
            model = self.superres.state_dict(),
        ...
        # determine if superresolution config is available

        if hasattr(self.superres, '_config'):
            self.print(f'this checkpoint is commandable from the CLI - "superresolution --model {str(path)} \"<prompt>\""')

            save_obj = {
                **save_obj,
                'superres_type': 'elucidated' if self.is_elucidated else 'original',
                'superres_params': self.superres._config
            }
    ...
    def load(self, path, only_model = False, strict = True, noop_if_not_exist = False):
        ...
        try:
            self.superres.load_state_dict(loaded_obj['model'], strict = strict)
        except RuntimeError:
            print("Failed loading state dict. Trying partial load")
            self.superres.load_state_dict(restore_parts(self.superres.state_dict(),
                                                      loaded_obj['model']))
    ...
    def use_ema_unets(self):
        ...
        self.reset_ema_unets_all_one_device()
        self.superres.reset_unets_all_one_device()

        self.unets.eval()

        trainable_unets = self.superres.unets
        self.superres.unets = self.unets                  # swap in exponential moving averaged unets for sampling

        output = yield

        self.superres.unets = trainable_unets             # restore original training unets
    ...
    def print_unet_devices(self):
        ...
        for i, unet in enumerate(self.superreso.unets):
    ...
    def encode_text(self, text, **kwargs):
        return self.superres.encode_text(text, **kwargs)
    ...
    def sample(self, *args, **kwargs):
        ...
        with context():
            output = self.superres.sample(*args, device = self.device, **kwargs)
        ...
        **kwargs
        ...
        for chunk_size_frac, (chunked_args, chunked_kwargs) in split_args_and_kwargs(*args, split_size = max_batch_size, **kwargs):
            with self.accelerator.autocast():
                loss = self.superres(*chunked_args, unet = self.unet_being_trained, unet_number = unet_number, **chunked_kwargs)
