...
# main superres ddpm class, which is a cascading DDPM from Ho et al.
...
class Superres(nn.Module):
    ...
    def p_mean_variance(
        ...
        *,
        ...
        assert not (cond_scale != 1. and not self.can_classifier_guidance), 'superres was not trained with conditional dropout, and thus one cannot use classifier free guidance (cond_scale anything other than 1)'
    ...
    def sample(
        ...
        assert not (self.condition_on_text and not exists(text_embeds)), 'text or text encodings must be passed into superres if specified'
        assert not (not self.condition_on_text and exists(text_embeds)), 'superres specified not to be conditioned on text, yet it is presented'
    ...
    def forward(
        ...
        **kwargs
        ...
        if exists(prev_image_size):
            if input_lowres!= None:
                lowres_cond_img=input_lowres
            else:
